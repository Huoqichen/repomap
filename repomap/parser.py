...
JAVASCRIPT_EXTENSIONS = tuple(sorted(LANGUAGE_EXTENSIONS["JavaScript"]))
...
def build_module_inventory(root: Path) -> tuple[list[ModuleInfo], list[LanguageSummary], str | None]:
    ...
    for file_path in source_files:
        ...
        extension_counter[language].add(file_path.suffix.lower())

    modules: list[ModuleInfo] = []
    python_files = [path for path in source_files if detect_language(path) == "Python"]
    javascript_files = [path for path in source_files if detect_language(path) == "JavaScript"]
    go_files = [path for path in source_files if detect_language(path) == "Go"]

    modules.extend(_analyze_python_modules(root, python_files))
    modules.extend(_analyze_javascript_modules(root, javascript_files))
...
def _analyze_javascript_modules(root: Path, javascript_files: list[Path]) -> list[ModuleInfo]:
    alias_index: dict[str, str] = {}
    path_to_id: dict[Path, str] = {}
    display_by_path: dict[Path, str] = {}

    for file_path in sorted(javascript_files):
        relative = file_path.relative_to(root)
        base_name = relative.with_suffix("").as_posix()
        module_id = f"javascript:{base_name}"
        display_name = base_name[:-6] if base_name.endswith("/index") else base_name
        resolved_path = file_path.resolve()
        path_to_id[resolved_path] = module_id
        display_by_path[resolved_path] = display_name
